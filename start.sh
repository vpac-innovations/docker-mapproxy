...
groupadd -f -g $GROUP_ID mapproxy
...
cd /mapproxy
cp /mapproxy.yaml ./mapproxy.yaml
rm -fv /tmp/mapproxy.pid
su $USER_NAME -c "mapproxy-util create --force -t wsgi-app -f mapproxy.yaml /mapproxy/app.py"
su $USER_NAME -c "uwsgi --protocol=http --ini /uwsgi.conf"
